...
def set_dataset_locations(tickers):
    ...
    for ticker in tickers:
        ...
        dataset_locs[ticker] = f"datasets\\{base}_15.csv"
    return dataset_locs
...
def last_close_avail_sim(simTime, ticker):
    ...
    try:
        df = data.loc[simTime-1801:simTime].copy()
        if df.empty or len(df) == 0:
            print(f"[WARNING] No data for {ticker} in last 15 min @ {simTime}")
...
def compileTargetTrend(unix_start, unix_end, c, target=True, close=False, predMulti=1):
    ...
    if target:
        # Calculate trend based on future 28 days
        indexer = pd.api.indexers.FixedForwardWindowIndexer(window_size=int((96*28)*predMulti))
        ...
        # Use atr2 (14-hour ATR) and scale by its 28-day rolling mean
        df2['avg_atr2'] = df2['atr2'].rolling(window=96*28, min_periods=1).mean()
        # Avoid division by zero; use 1 if avg_atr2 is 0 or NaN
        df2['atr_scale'] = df2['atr2'] / df2['avg_atr2'].replace(0, 1)
        # Base threshold is 1% (0.01); scale it by ATR ratio, cap at reasonable bounds
        # df2['threshold'] = 0.01 * df2['atr_scale'].clip(lower=0.5, upper=1.5)

        # df2['threshold2'] = df2['threshold'] * df2['close']

        # Base threshold off simple 0.3%
        df2['threshold'] = 0.01 * predMulti

        # Calculate trend: up (1) if nxtmean exceeds close + threshold, down (-1) if below close - threshold
        df2['trendup'] = df2.apply(lambda x: bool(x.nxtmean >= (x.close + (x.close * x.threshold))), axis=1)

        df2['trenddown'] = df2.apply(lambda x: bool(x.nxtmean <= (x.close - (x.close * x.threshold))), axis=1)

        df2['trend'] = df2.apply(lambda x: (int(x.trendup) + (int(x.trenddown) * -1)), axis=1)

        # df2['trend'] = df2.apply(lambda x: int(x.trendup) * -1, axis=1)

        # Remove temporary columns
        df2 = df2.drop(['nxtmean', 'avg_atr2', 'atr_scale', 'threshold', 'trendup', 'trenddown'], axis=1)

        # df2 = df2.drop(['avg_atr2', 'atr_scale', 'threshold', 'trendup', 'trenddown'], axis=1)
...
def compiled_data(ticker):
    ...
    df['dayema1'] = df['close'].ewm(span=4*24*12).mean()  # ~12 days
    df['dayema2'] = df['close'].ewm(span=4*24*26).mean()  # ~26 days
    df['dayema3'] = df['close'].ewm(span=4*24*9).mean()   # ~9 days
    df['hrema1'] = df['close'].ewm(span=4*12).mean()      # ~12 hours
    df['hrema2'] = df['close'].ewm(span=4*26).mean()      # ~26 hours
    df['hrema3'] = df['close'].ewm(span=4*9).mean()       # ~9 hours
    df['hrema4'] = df['close'].ewm(span=4*6).mean()       # ~6 hours
    df['hrema5'] = df['close'].ewm(span=4*3).mean()       # ~3 hours
    df['hrema6'] = df['close'].ewm(span=4).mean()       # ~1 hours

    # Compute Relative Strength Index (RSI)
    df['rsi1'] = rsi(df)                     # ~14 minutes
    df['rsi2'] = rsi(df, periods=14*4)      # ~14 hours
    df['rsi3'] = rsi(df, periods=14*(96))    # ~14 days

    # Compute Money Flow Index (MFI)
    df['mfi1'] = mfi(df['high'], df['low'], df['close'], df['volume'])         # ~14 minutes
    df['mfi2'] = mfi(df['high'], df['low'], df['close'], df['volume'], n=4*14)  # ~14 hours
    df['mfi3'] = mfi(df['high'], df['low'], df['close'], df['volume'], n=4*24*14)  # ~14 days

    # Compute price change deltas
    df['delta_p_1h'] = (df["close"] - df["open"].shift(4*1)) / df["open"].shift(4*1)      # 1 hour
    df['delta_p_8h'] = (df["close"] - df["open"].shift(4*8)) / df["open"].shift(4*8)      # 8 hours
    df['delta_p_day'] = (df["close"] - df["open"].shift(4*24)) / df["open"].shift(4*24)   # 1 day
    df['delta_p_week'] = (df["close"] - df["open"].shift(4*24*7)) / df["open"].shift(4*24*7)  # 1 week
    df['delta_p_month'] = (df["close"] - df["open"].shift(4*24*30)) / df["open"].shift(4*24*30)  # 1 month
    ...
    for period, prefix in [(20, 'bb1'), (20*4, 'bb2'), (20*4*24, 'bb3')]:
    ...
    df['atr1'] = df['true_range'].rolling(1, min_periods=1).mean()             # ~14 minutes
    df['atr2'] = df['true_range'].rolling(4*14, min_periods=1).mean()          # ~14 hours
    df['atr3'] = df['true_range'].rolling(96*14, min_periods=1).mean()        # ~14 days


    # Stochastic Oscillator
    for period, prefix in [(2, 'stoch1'), (14*4, 'stoch2'), (14*96, 'stoch3')]:
    ...
    # Volume Deltas
    df['delta_v_1h'] = df['volume'] / df['volume'].shift(4)      # 1-hour volume change
    df['delta_v_8h'] = df['volume'] / df['volume'].shift(4*8)    # 8-hour volume change
    df['delta_v_day'] = df['volume'] / df['volume'].shift(4*24)  # 1-day volume change
